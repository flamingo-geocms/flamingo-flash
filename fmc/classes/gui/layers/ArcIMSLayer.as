...
class gui.layers.ArcIMSLayer extends AbstractPositionable{
	...
	*/
	function setConfig(xml:Object) {
		...
		lConn.onGetServiceInfo = function(extent, servicelayers, objecttag, requestid) {
			...
			Logger.console("ArcIMSLayer.setConfig()1");
			...
			//thisObj.update();
			Logger.console("ArcIMSLayer.setConfig()2");
			thisObj.flamingo.raiseEvent(thisObj, "onGetServiceInfo", thisObj);
			Logger.console("ArcIMSLayer.setConfig()3");
		...
		if (this.initService==true){
			...
			if(!serviceInfoRequestSent){
				...
				//_global.flamingo.tracer(_global.flamingo.getId(this) + " naar getServiceInfo " + mapservice);
				Logger.console("ArcIMSLayer do getServiceInfo");
	...
	*/
	...
	function _update(nrtry:Number):Void {
		Logger.console("ArcIMSLayer._update");
		if (! visible || ! map.visible) {
		Logger.console("ArcIMSLayer._update6");
			_visible = false;
			return;
		}
		...
		//}		
		if (updating) {
		Logger.console("ArcIMSLayer._update7");
			return;
		}
		if (server == undefined) {
		Logger.console("ArcIMSLayer._update8");
			return;
		}
		if (mapservice == undefined) {
		Logger.console("ArcIMSLayer._update9");
			return;
		}
		if (! map.hasextent) {
		Logger.console("ArcIMSLayer._update0");
			return;
		}
		...
		thisObj._starttimeout();		
		Logger.console("ArcIMSLayer._update_finished");
	}
	function _starttimeout() {
		Logger.console("*****start timeout");
		clearInterval(timeoutid);
		timeoutid = setInterval(this, "_timeout", (timeout*1000));
	}
	function _stoptimeout() {
		Logger.console("*****_stoptimeout");
		clearInterval(timeoutid);
	}
	function _timeout() {		
		Logger.console("*****_timeout");
		clearInterval(timeoutid);
		updating = false;
	...
	*/
	...
	public function getParent():Object {
		Logger.console("GetParent() called: "+this.map);
		return this.map;
	}
	/**
