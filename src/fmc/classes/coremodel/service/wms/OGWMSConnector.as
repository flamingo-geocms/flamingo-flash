...
dynamic class coremodel.service.wms.OGWMSConnector {
			...
			xrequest.onLoad = function(success:Boolean) {
				...
				if (success) {
					if (this.firstChild.nodeName.toLowerCase() == "serviceexceptionreport") {
						...
						thisObj.events.broadcastMessage("onError", thisObj.error, obj, thisObj.requestid);
					} else {
						thisObj.events.broadcastMessage("onResponse", thisObj);
					...
					thisObj.events.broadcastMessage("onResponse", thisObj);
					thisObj.events.broadcastMessage("onError", thisObj.error, obj, thisObj.requestid);
					//thisObj.events.broadcastMessage("onError", thisObj.error, obj, 1);
				}
				thisObj.busy = false;
				// do some cleaning
				delete this;
	...
	private function _process_featureInfoResponseWithAttributesInFieldsTag(xml, obj, reqid){
		...
		for (var i:Number = 0; i<featureObjects.length; i++) {
			...
			if(attributeObjects.length == 0){
				for (var attr in featureObjects[i].attributes) {
	...
	private function _process_featureInfoResponse(xml:XML, obj, reqid) {
		...
		for (var i:Number = 0; i<xfields.length; i++) {
			var feature:Object = new Object();
			layer = xfields[i].attributes._LAYERID_;
			if (layer == undefined) {
				layer = "?";
			}
