...
function execute() {
	...
	if (schemaStr == null) { alert(">> ERROR: Could not read the schema file."); return; }
	...
	if (schema.length < 1) { alert(">> ERROR: Schema is empty or could not be read."); return; }
...
function compile(p_fileURI,p_profile,p_fileName) {
	...
	doc.publish();
	appendToLog("");
	fl.outputPanel.save(dirURI+"compile_log.txt",true);
	fl.outputPanel.clear();
	fl.closeDocument(fl.documents[0],false); // close fla without saving
}

function move(p_file,p_newName,p_fileName,type) {
	...
	if (FLfile.exists(p_newName) == true) {
		...
		if(type=="swf"){
			...
			if (!ok) { appendToLog(">> ERROR: Original file not deleted: "+p_fileName); error=true; }
		}
		return true;
	}
}
...
function appendToLog(p_str) {
	log += "\n"+p_str;
	FLfile.write(dirURI+"compile_log.txt","\n"+p_str,"append");
}
function startLog(p_str) {
	log = p_str;
	FLfile.write(dirURI+"compile_log.txt",p_str);
}